 ...
 *
 * Description:
 *    Clear specific screen pages
 *
 * Params:
 *   [in] color - color to fill the screen with
 *   [in] startPage - 0xB0 to 0xB6
 *   [in] endPage - 0xB
 *
 *****************************************************************************/
void oled_clearPages(oled_color_t color, uint8_t startPage, uint8_t endPage)
{
    uint8_t i;
    uint8_t c = 0;
    ...
    for( i=startPage; i<endPage; i++ ) {            // Go through all 8 pages
        setAddress(i,0x00,0x10);
        writeDataLen(c, 132);
    }
